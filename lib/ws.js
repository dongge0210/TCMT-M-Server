...
// Single-use, short-TTL tickets for browser WebSocket auth. The browser
// WebSocket API cannot set the Authorization header, and long-lived tokens
// must not sit in URLs — a one-time ticket bounds any log exposure to ~30s.
export function createTicketStore({ ttlMs = 30000 } = {}) {
...
export function acceptUpgrade(req, socket, onClient, onClose, { authToken = '', tickets = null } = {}) {
  ...
  if (authToken) {
    ...
    authorized = provided === authToken;
  }
  // Long-lived token via query param remains supported for old clients;
  // new clients authenticate with a single-use ticket (?ticket=...).
  if (authorized && tickets) {
    authorized = tickets.consume(url.searchParams.get('ticket') || '');
  }
  if (!authorized) {
    socket.write(
