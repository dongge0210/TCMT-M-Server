...
// Long-history read: Flux query against the archive bucket, parsed from
// Influx's annotated CSV into [{ts, value}].
export function queryInflux({ influxUrl, token, org, bucket, deviceId, field, from, to, bucketSec = 3600 }) {
  return new Promise((resolve, reject) => {
    const url = new URL(influxUrl);
    const lib = url.protocol === 'https:' ? https : http;
    const flux = [
      ...
      `  |> filter(fn: (r) => r._measurement == "tcmt_metric" and r.device_id == "${deviceId}" and r._field == "${field}")`,
      `  |> aggregateWindow(every: ${bucketSec}s, fn: mean, createEmpty: false)`,
    ].join('\n');
    const req = lib.request({
      hostname: url.hostname,
      port: url.port || (url.protocol === 'https:' ? 443 : 80),
      ...
      res.on('end', () => {
        if (res.statusCode < 200 || res.statusCode >= 300) {
          reject(new Error(`influx query HTTP ${res.statusCode}`));
          return;
        }
        try { resolve(parseAnnotatedCsv(body)); } catch (e) { reject(e); }
      });
    });
    req.on('error', reject);
    req.on('timeout', () => req.destroy(new Error('influx query timeout')));
    req.end(flux);
  });
}
...
function parseAnnotatedCsv(csv) {
  ...
  const header = lines.find(l => l.startsWith(',result'));
  if (!header) return [];
  const cols = header.split(',');
  const timeIdx = cols.indexOf('_time');
  const valIdx = cols.indexOf('_value');
  if (timeIdx < 0 || valIdx < 0) return [];
  const out = [];
  for (const line of lines) {
    // Data rows start with an empty annotation column (leading comma) —
    // only the '#' comment rows are skipped. The header row itself parses
    // to NaN values and is filtered by the isFinite checks below.
    if (!line || line.startsWith('#')) continue;
    const parts = line.split(',');
    if (valIdx >= parts.length || parts[valIdx] === '') continue;
    const ts = Date.parse(parts[timeIdx]);
    const value = Number(parts[valIdx]);
    if (Number.isFinite(ts) && Number.isFinite(value)) out.push({ ts, value });
  }
  return out;
}
