...
// Default alert thresholds (used until the user configures their own).
export const DEFAULT_ALERT_RULES = [
  { field: 'cpu_usage', op: '>', value: 90 },
  { field: 'cpu_temp', op: '>', value: 85 },
  { field: 'gpu_usage', op: '>', value: 95 },
  { field: 'gpu_temp', op: '>', value: 90 },
  { field: 'memory_percent', op: '>', value: 95 },
];
...
export class Store {
  ...
  _ensureColumn(table, column, ddl) {
    ...
    console.log(`[store] added ${table}.${column}`);
  }
  ...
  _ensureTables() {
    ...
    `);
    // Device management columns (added incrementally for older DBs).
    this._ensureColumn('devices', 'display_name', "TEXT NOT NULL DEFAULT ''");
    this._ensureColumn('devices', 'grp', "TEXT NOT NULL DEFAULT ''");
    this._ensureColumn('devices', 'note', "TEXT NOT NULL DEFAULT ''");
  }

  _loadDevices() {
    const rows = this.db.prepare(
      'SELECT id, client_key, name, display_name, grp, note, os, model, token, last_seen FROM devices'
    ).all();
  ...
  _insertDeviceRow(d) {
    ...
    this.db.prepare(`
      INSERT OR REPLACE INTO devices
        (id, client_key, name, display_name, grp, note, os, model, token, created_at, last_seen)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(d.id, d.clientKey, d.name, d.displayName || '', d.group || '', d.note || '',
           d.os, d.model, d.token, d.lastSeen, d.lastSeen);
  }
  ...
  // Rename / regroup / annotate a device (persisted).
  updateDevice(id, { displayName, group, note } = {}) {
    const d = this.devices.get(id);
    if (!d) return null;
    if (displayName !== undefined) d.displayName = String(displayName).trim();
    if (group !== undefined) d.group = String(group).trim();
    if (note !== undefined) d.note = String(note);
    this.db.prepare(
      'UPDATE devices SET display_name = ?, grp = ?, note = ? WHERE id = ?'
    ).run(d.displayName, d.group, d.note, d.id);
    return d;
  }

  // ── server config (key/value, persisted) ─────────────
  ...
  // Alert rules ({rules: [...]}), user-configurable; defaults until set.
  alertRules() {
    const raw = this.getConfig('alerts', '');
    if (raw) {
      try {
  ...
  setAlertRules(rules) {
    this.setConfig('alerts', JSON.stringify({ rules }));
  }

  // Current alert violations across online devices (pure computation —
  // cheap enough to run on the 0.5s broadcast tick).
  alerts() {
    const rules = this.alertRules();
    const active = [];
    const now = Date.now();
    for (const d of this.devices.values()) {
      if (now - d.lastSeen >= ONLINE_MS) continue;
      const latest = d.latest || {};
      for (const r of rules) {
        let v;
        if (r.field === 'memory_percent') {
          if (latest.memory_total && latest.memory_used !== undefined) {
            v = (latest.memory_used / latest.memory_total) * 100;
          }
        } else if (typeof latest[r.field] === 'number') {
          v = latest[r.field];
        }
        if (v === undefined) continue;
        const hit = (r.op === '>' && v > r.value) || (r.op === '<' && v < r.value);
        if (hit) {
          active.push({
            deviceId: d.id,
  ...
  // Cross-device comparison: same field, aligned bucket grid, one series
  // per device (bucket downsampling yields identical ts per device).
  compare(field, deviceIds, from, to, limit = 1000, bucket = 30) {
    const series = [];
    for (const id of deviceIds) {
      const d = this.devices.get(id);
      if (!d) continue;
      const points = this.history(id, field, from, to, limit, bucket);
      if (!points) continue;
      series.push({
        deviceId: id,
  ...
  // CSV export of one field's history.
  exportCsv(id, field, from, to) {
    const d = this.devices.get(id);
    const points = this.history(id, field, from, to, 0);
    if (!d || !points) return null;
    const rows = ['timestamp_iso,value'];
    for (const p of points) {
      rows.push(`${new Date(p.ts).toISOString()},${p.value !== undefined ? p.value : p.avg}`);
    }
    return { name: d.displayName || d.name, field, csv: rows.join('\n') + '\n' };
  }
  ...
  // Archiver support: drain timeseries rows older than the cutoff (bounded
  // batch) so they can be written to long-term storage first, then delete.
  // Returns { rows: [{deviceId, field, ts, value}], remaining } — call in a
  // loop until remaining is 0.
  drainOldTimeseries(cutoff, limit = 5000) {
    const rows = this.db.prepare(`
      SELECT device_id AS deviceId, field, ts, value
      FROM timeseries WHERE ts < ? ORDER BY ts ASC LIMIT ?
    `).all(cutoff, limit);
    if (!rows.length) return { rows, remaining: 0 };
    const ids = rows.map(r => r.ts);
    const placeholders = ids.map(() => '?').join(',');
    this.db.prepare(`DELETE FROM timeseries WHERE ts IN (${placeholders})`).run(...ids);
    return { rows, remaining: this.db.prepare(
      'SELECT COUNT(*) AS n FROM timeseries WHERE ts < ?').get(cutoff).n };
  }
