  ...
  return async function handle(req, res) {
    ...
    try {
      ...
      if (req.method === 'GET' && deviceMatch) {
        ...
        if (sub === 'archive') {
          if (!serverInfo.queryArchive) {
            throw httpError(501, 'archiving not configured (start server with --influx-url)');
          }
          const field = url.searchParams.get('field') || '';
          if (!field) throw httpError(400, "missing 'field' query parameter");
          const from = parseTime(url.searchParams.get('from'), Date.now() - 90 * 86400000);
          const to = parseTime(url.searchParams.get('to'), Date.now());
          const bucket = Math.max(60, Math.min(604800, Number(url.searchParams.get('bucket')) || 3600));
          const points = await serverInfo.queryArchive({
            deviceId: id, field, from, to, bucketSec: bucket,
          });
          return json(res, 200, { deviceId: id, field, from, to, count: points.length, history: points });
        }
        if (sub === 'export') {
          const field = url.searchParams.get('field') || '';
          if (!field) throw httpError(400, "missing 'field' query parameter");
